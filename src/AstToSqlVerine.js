export class AstToSqlVerine {
  ...
  createCondition(condition, parentType) {
    ...
    switch (condition.left.type) {
      case "STRING_FUNCTION":
        spanLeft = this.createStringFunction(
          condition.left,
          0,
    ...
    if (condition.right != undefined) {
      if (Array.isArray(condition.right)) {
        ...
        condition.right.forEach((inputField, index) => {
          ...
          if (index == 0) {
            ...
            spanInVal.setAttribute(
              "data-next-element",
              this.htmlElementCount - 3
            );
          } else {
        ...
        spanRightBracket.classList.add(
          this.getNextCodeElement(),
          "btnRightBracket",
          "synBrackets",
          "sqlIdentifier",
        ...
        switch (condition.right.type) {
          case "STRING_FUNCTION":
            conditionCount++;
            spanRight = this.createStringFunction(
              condition.right,
              0,
              parentType + "_" + conditionCount
            );
          ...
          case "INPUT":
            ...
            spanRight.innerHTML = this.getInputNumberText(
      ...
      spanRightFrom.innerHTML = this.getInputNumberText(
        condition.rightFrom.value
      );
    ...
    element.selectFields.forEach((selectField, idx) => {
      ...
      switch (selectField.type) {
        ...
        case "AGGREGATE":
          ...
          break;
        case "STRING_FUNCTION":
          const stringFunctionSpan = this.createStringFunction(
            selectField,
            idx,
            "SELECT_SELECT"
          );
        ...
        default:
          console.log(
            "Selectfield of type " + selectField.type + " canot be parsed."
          );
      }
    });
  ...
  createInsert(element, currentCodeline) {
    ...
    spanCreateInsert.classList.add(
      this.getNextCodeElement(),
      "synSQL",
      "parent",
      "sqlIdentifier"
    ...
    element.selectFields1.forEach((selectField, index) => {
      ...
      if (index > 0) {
        const elementGroupNumbers = this.getElementGroupNumbers(
          colSpan,
          numberOfValues,
          "insertColumn"
        );
  ...
  getElementGroupNumbers(currentElement, numberOfValues, type) {
    ...
    const colSpanCorrespondingValueKomma =
      parseInt(colSpanElementNumber) + kommaNumber;
    const colSpanCorrespondingValueInputField =
      parseInt(colSpanElementNumber) + selectFieldNumber;
    const elementGroupNumbers = [
      colSpanKommaBefore,
      colSpanCorrespondingValueKomma,
      colSpanCorrespondingValueInputField,
    ];
  ...
  createCreateColumn(element, currentCodeline, lastElement) {
    ...
    spanCreateColumn.append(this.createLeerzeichen());
    spanCreateColumn.append(this.createLeerzeichen());
    spanCreateColumn.classList.add(
      this.getNextCodeElement(),
      "synSQL",
      "parent",
      "sqlIdentifier"
  ...
  getTableFromColumn(selectField) {
    if (
      ...
      selectField.value.split(".").length > 1
    ) {
  ...
  createColumn(selectField, idx, sqlDataElement) {
    ...
    spanColumn.setAttribute("data-sql-element", sqlDataElement);
    spanColumn.innerHTML = selectField.value;
    spanColumn.classList.add(
      this.getTableFromColumn(selectField),
      "selColumn",
      "synColumns"
    );
    return spanColumn;
  ...
  createAggregate(selectField, idx, sqlDataElement) {
    ...
    innerSpan.classList.add(
      this.getNextCodeElement(),
      this.getTableFromColumn(selectField.selectField),
      "selColumn",
      "synColumns",
      "sqlIdentifier",
      "inputFields",
  ...
  createStringFunction(selectField, idx, sqlDataElement) {
    ...
    if (selectField.selectFields.length > 1) {
      selectField.selectFields.forEach((selField, index) => {
        ...
        if (index < 1) {
          innerSpan.setAttribute(
            "data-sql-element",
            sqlDataElement + "_" + stringFunction + "_FUNCTION_1"
          );
          innerSpan.innerHTML = selField.value;
          innerSpan.classList.add(
            this.getNextCodeElement(),
            this.getTableFromColumn(selField),
            "selColumn",
            "synColumns",
            "sqlIdentifier",
            "inputFields",
          ...
          innerSpan.setAttribute(
            "data-sql-element",
            sqlDataElement + "_" + stringFunction + "_FUNCTION_2"
          );
      ...
      innerSpan.setAttribute(
        "data-sql-element",
        "SELECT_SELECT_" + stringFunction + "_FUNCTION"
      );
      innerSpan.innerHTML = selectField.selectFields[0].value;
      innerSpan.classList.add(
        this.getNextCodeElement(),
        this.getTableFromColumn(selectField.selectFields[0]),
        "selColumn",
        "synColumns",
        "sqlIdentifier",
        "inputFields",
  ...
  createAS(selectField, idx) {
    ...
    if (selectField.selectField1.type == "STRING_FUNCTION") {
      /*
                        ...
                        */
      spanColumn = this.createStringFunction(
        selectField.selectField1,
        idx,
        "SELECT_SELECT"
      );
  ...
  createLeerzeichenMitKomma() {
    ...
    spanLeerzeichenKomma.classList.add(
      this.getNextCodeElement(),
      "leerzeichen"
    );
